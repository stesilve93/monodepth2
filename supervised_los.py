...
from networks import DepthDecoder, ResnetEncoder, RangeDecoder, LOSDecoder
...
full_dataset = LOSDataset(database_dir, img_size=img_size, source=source_depth, normalize_maps=False)
...
range_decoder = LOSDecoder(num_ch_enc=encoder.num_ch_enc)  # Load the depth decoder
...
range_decoder = range_decoder.to(device)
...
def evaluate_model(loader, encoder, range_decoder, loss_fn, device):
    encoder.eval()  # Set encoder to evaluation mode
    range_decoder.eval()  # Set decoder to evaluation mode
    total_loss = 0
    total_mae = 0
    total_rmse = 0
    count = 0
    ...
    with torch.no_grad():  # Disable gradient computation
        for batch in loader:
            ...
            gt_los_range = batch["los-range"].to(device)
            gt_los_range.squeeze(1)
            ...
            pred_range = outputs
            pred_range.squeeze(1)

            gt_los_range = gt_los_range.float()  # To make it shape [4, 1]
            loss = loss_fn(pred_range, gt_los_range)
            #loss = torch.mean(torch.abs((pred_range - gt_los_range) / gt_los_range))

            # Convert to list for easy printing
            gt = [float(x) for x in gt_los_range.view(-1)]
            pred = [float(x) for x in pred_range.view(-1)]


            print(f"  u ground truth: {gt[0]:.6f}")
            print(f"  u predicted   : {pred[0]:.6f}")
            print(f"  v ground truth: {gt[1]:.6f}")
            print(f"  v predicted   : {pred[1]:.6f}")
            print(f"  r ground truth: {gt[2]:.3f} km")
            print(f"  r predicted   : {pred[2]:.3f} km")
            print('\n')

            total_loss += loss.item()

            mae = torch.mean(torch.abs(pred_range - gt_los_range))
            rmse = torch.sqrt(torch.mean((pred_range - gt_los_range) ** 2))
...
for epoch in range(num_epochs):
    ...
    range_decoder.train()  # Set decoder to training mode
    ...
    for step, batch in enumerate(train_loader):
        ...
        pred_range = outputs # Output at the highest resolution
        pred_range.squeeze(1)
        ...
        #     writer.add_images("Train/Predicted Depth", pred_range, epoch)
        #     writer.add_images("Train/Ground Truth Range", gt_los_range, epoch)

    # Validation
    val_loss, val_mae, val_rmse = evaluate_model(val_loader, encoder, range_decoder, loss_fn, device)
    ...
    # Test evaluation
    test_loss, test_mae, test_rmse = evaluate_model(test_loader, encoder, range_decoder, loss_fn, device)
...
torch.save(range_decoder.state_dict(), model_path+save_path+"/range.pth")
print(f"Decoder model saved to {model_path+save_path}/range.pth")
