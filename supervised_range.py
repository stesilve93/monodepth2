...
full_dataset = RangeDataset(img_dir, labels_dir, img_size=img_size, source=source_depth, normalize_maps=False)
...
range_decoder = RangeDecoder(num_ch_enc=encoder.num_ch_enc)  # Load the depth decoder
...
def evaluate_model(loader, encoder, range_decoder, loss_fn, device):
    ...
    with torch.no_grad():  # Disable gradient computation
        for batch in loader:
            ...
            rmse = torch.sqrt(torch.mean((pred_range - gt_range) ** 2))
...
for epoch in range(num_epochs):
    ...
    for step, batch in enumerate(train_loader):
        ...
        #     writer.add_images("Train/Ground Truth Range", gt_range, epoch)
